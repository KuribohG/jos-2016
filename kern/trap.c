...
page_fault_handler(struct Trapframe *tf)
{
    ...
    if (curenv->env_pgfault_upcall) {
        struct UTrapframe *addr = (struct UTrapframe *) (UXSTACKTOP - sizeof(struct UTrapframe));
        if (tf->tf_esp >= UXSTACKTOP - PGSIZE && tf->tf_esp <= UXSTACKTOP - 1) {
            addr = (struct UTrapframe *) (tf->tf_esp - sizeof(struct UTrapframe) - 4);
        }
        ...
        *addr = utf;
        ...
        curenv->env_tf.tf_esp = (uint32_t) addr;
        env_run(curenv);
    }
